...
# We are only interested in api builds, which are triggered when lcc_frontend_toolkit master is checked in.
...
wget https://github.com/lccgov/lcc_frontend_toolkit/archive/master.tar.gz -O new-toolkit.tar.gz

tar -xzf new-toolkit.tar.gz

cd lcc_frontend_toolkit-master/
...
rm -r lcc_frontend_toolkit-master
rm new-toolkit.tar.gz

VERSION_LATEST=`cat VERSION.txt`
VERSION_REGISTRY=`npm view lcc_frontend_toolkit version`
